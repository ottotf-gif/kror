...
import { Send, CheckCircle, Phone, Mail, MapPin, Loader2, Map as MapIcon, ExternalLink } from 'lucide-react';
...
// Karta som bara laddas när besökaren gett samtycke till Google-cookies.
function MapEmbed() {
  const { mapsAllowed, grant } = useConsent();
  ...
  return (
    <div className="rounded-xl border border-ink/10 bg-sand-200 h-[220px] flex flex-col items-center justify-center text-center px-6 gap-3">
      <MapIcon size={26} className="text-ink-muted" />
      <p className="text-sm text-ink-soft max-w-xs">
        Kartan laddas via Google och sätter cookies. Tillåt så visas den här.
      </p>
      <div className="flex items-center gap-4">
        <button
          onClick={grant}
          className="text-sm font-semibold text-marine hover:text-copper transition-colors"
        >
          Visa karta
        </button>
        <a
          ...
          className="inline-flex items-center gap-1.5 text-sm text-ink-soft hover:text-copper transition-colors"
        >
          Öppna i Google Maps
